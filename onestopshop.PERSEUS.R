...
correct.names <- c("cust", "PlateID", "lysed.inits", "lysed.dates", "Inc.temp", "reag.pro", 
                   "lysis.buff", "clean.inits", "clean.dates", "reag.beads", "reag.buff", "reag.w1", "reag.w2", "reag.el", "Mag")
...
#  {
    ...
    write.csv(df.final[,-c(7:9)], file = paste("Trimmed and formatted NGF ", unformatted.file, sep = "", ".csv"), row.names = F)
    print(paste(sub("Genotyping", "Trimmed NGF ", unformatted.file),sep = "", ".csv"))
    print(paste("completed", a, "of", length(file.list)))
  } #file writing
...
for(i in 1:length(well.list)){
  loop.df.subset <- df.final[df.final$MasterWell == well.list[i],]
  raw.output.table[i,1] <- as.character(well.list[i])
  raw.output.table[i,2] <- as.numeric(length(loop.df.subset$Call[loop.df.subset$Call %in% uncalled])/length(loop.df.subset$MasterWell))
...
plate.pass.rate <- heatmap.2(x = output.matrix, Rowv = FALSE, Colv = FALSE, dendrogram = "none",
                             ...
                             main = paste(length(unique(df.final$DaughterPlate)),"unique arrays"), col = colorRampPalette(c("white", "red"))(10))
...
} #this is for bulk processed files

######
######summary tables
######
{#I actually think this works now. No way it's that easy.
  proj.list <- sort(unique(df.final$Project))
  ...
  plate.table <- data.frame(matrix(NA, nrow = length(plate.list), ncol = 5))
  colnames(plate.table) <- c("plate BC", "% failed", "average distance", "normalized distance", "project(s)")
  ...
  project.table <- data.frame(matrix(NA, nrow = length(proj.list), ncol = 4))
  colnames(project.table) <- c("project", "% failed", "average distance", "normalized distance")
  project.table$project <- proj.list
  
  sample.table <- data.frame(matrix(NA, nrow = length(sample.list), ncol = 6))
  colnames(sample.table) <- c("sample", "Plate", "% failed", "QC pass rate", "average distance", "normalized distance")
  ...
  for(i in 1:length(mp.well.list)){
    ...
    loop.df.subset.qqc <- df.master.list[df.master.list$MasterWell == mp.well.list[i],]
    mp.raw.output.table[i,2] <- 100*as.numeric(length(loop.df.subset$Call[loop.df.subset$Call %in% uncalled])/length(loop.df.subset$MasterWell))
    mp.raw.output.table[i,3] <- as.numeric(mean(loop.df.subset.qqc$distance, na.rm = T))
    mp.raw.output.table[i,4] <- as.numeric(mean(loop.df.subset.qqc$distance.norm, na.rm = T))
    
    print(length(mp.well.list)-i)
  } 
  if(mp.raw.output.table[1,2] == "100") {
   mp.raw.output.table <- mp.raw.output.table[-c(1),]
    }
  ...
  for(i in 1:length(plate.list)){
    ...
    plate.table[i,3] <- as.numeric(mean(loop.df.subset.qqc$distance, na.rm = T))
    plate.table[i,4] <- as.numeric(mean(loop.df.subset.qqc$distance.norm, na.rm = T))
    if(length(unique(loop.df.subset.qqc$Project)) == 1) {
      plate.table[i,5] <- unique(loop.df.subset.qqc$Project)
    } else {
      plate.table[i,5] <- unique(loop.df.subset.qqc$Project)[2]  
    }
  ...
  for(i in 1:length(snp.list)){
    ...
    snp.table[i,3] <- as.numeric(mean(loop.df.subset.qqc$distance, na.rm = T))
    snp.table[i,4] <- as.numeric(mean(loop.df.subset.qqc$distance.norm, na.rm = T))
    print(length(snp.list)-i)
  }
  
  #this may or may not work
  for(i in 1:length(proj.list)){
    loop.df.subset <- df.final[df.final$Project == proj.list[i],]
    loop.df.subset.qqc <- df.master.list[df.master.list$Project == proj.list[i],]
    project.table[i,2] <- 100*as.numeric(length(loop.df.subset$Call[loop.df.subset$Call %in% uncalled])/length(loop.df.subset$MasterPlate))
    project.table[i,3] <- as.numeric(mean(loop.df.subset.qqc$distance, na.rm = T))
    project.table[i,4] <- as.numeric(mean(loop.df.subset.qqc$distance.norm, na.rm = T))
    print(length(proj.list)-i)
  }
  
  raw.output.table <- mp.raw.output.table
...
###### 
{
  ...
  hist(plate.table$`normalized distance`, breaks = 50, main = "std distribution of cluster tightness", xlab = "tightness")
...
for(i in 1:length(MGBD.names)){
  if(i == 1){
    ...
    if(length(input.file) == 14){
      ...
      input.file.temp <- cbind(input.file.temp, input.file[,5:15])
...
#####
{
  ...
  ##this if for adding in project. If it borks, comment out.
  ...
  # plate.table$chem <- NA
  # plate.table$chem <- ID003$chem[match(plate.table$`plate BC`, ID003$PlateID, incomparables = NULL)]; unique(plate.table$chem)
  # plate.table$chem <- ID003.IDBD$chem[match(plate.table$`plate BC`, ID003.IDBD$PlateID, incomparables = NULL)]; unique(plate.table$chem)
  
  plate.table.IDBD <- plate.table[plate.table$`plate BC` %in% ID003.IDBD$PlateID,]
  plate.table.IDBD$chem <- "IDBD"
  plate.table.IDBD$Mag <- ID003.IDBD$Mag[match(plate.table.IDBD$`plate BC`, ID003.IDBD$PlateID)]
  plate.table.Mag <- plate.table[plate.table$`plate BC` %in% ID003$PlateID,]
  plate.table.Mag$chem <- "Mag mini"
  plate.table.Mag$Mag <- ID003$Mag[match(plate.table.Mag$`plate BC`, ID003$PlateID)]
  plate.table.new <- rbind(plate.table.Mag, plate.table.IDBD)
  plate.table.new$pass.rate <- 100-plate.table.new$`% failed`
  plate.table <- plate.table.new
  ...
  #ID003.complete <- ID003[!is.na(ID003$reag.pro),]
...
#par(mar = c(1,1,1,1))
  ...
  ##
  ggplot(ID003.magnets.A, aes(x = factor(Mags), y = pass.rate, fill = factor(Mags))) + 
    geom_boxplot() +
    facet_grid(rows = factor(ID003.magnets.A$clean.inits))
  
  ggplot(ID003.magnets.A, aes(x = factor(clean.inits), y = pass.rate, fill = factor(clean.inits))) + 
    geom_boxplot() +
    facet_wrap( ~ Mags, ncol = 4)
  ##
  ...
  ggplot(plate.table.new, aes(x = factor(chem), y = pass.rate, fill = factor(chem))) + 
...
ggplot(ID003.complete, aes(x = factor(spec.cust), y = pass.rate, fill = factor(spec.cust))) + 
  #scale_fill_brewer(palette="Set3") + 
  geom_boxplot() +
  #geom_violin(trim=FALSE) + 
  stat_summary(fun.data = n_fun, geom = "text", position = position_dodge(width = 0.75)) 
...
#################################GLM shit
#################################
# ID003.glm <- ID003.complete[,-c(1, 2, 5, 17, 18)] #take out dist/dist.norm because they explain so much of the variation, not necessarily informative
ID003.glm <- ID003.cleaned[,-c(1, 2, 5, 17, 18, 19)] #take out dist/dist.norm because they explain so much of the variation, not necessarily informative
