...
#plotting
  ...
  asdf <- dunnTest(plate.table.magnet$pass.rate ~ plate.table.magnet$Mag, method = "holm")[["dtres"]]; describeBy(plate.table.magnet$pass.rate, group = plate.table.magnet$Mag)
  
  #
  dunnTest(ID003.magnets.CLB$pass.rate ~ ID003.magnets.CLB$Mags, method = "holm")[["dtres"]]; describeBy(ID003.magnets.CLB$pass.rate, group = ID003.magnets.CLB$Mags)
  #
...
#################################GLM shit
#################################
ID003.glm <- ID003.cleaned[,-c(1, 2, 5, 15, 17, 18, 19, 20)] #take out dist/dist.norm because they explain so much of the variation, not necessarily informative
