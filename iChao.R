iChao <- function(){
  ...
  #1.0 7/23/19: added primal test to prevent breaking. 
    #Of course it'll still output numbers if there are double sampled primals. 
    #if there aren't any doubletons in a mince task it won't do math, so hopefully this doesn't happen often. 
    #minor cosmetic tweaks. ongoing.  
    #TO DO:
      #pull data from AOF
      #do more of the basic maths internally so the output is enough to work from.
...
#   ##expected maths
#   output$'% expected'[2:3] <-output$`% found`[1]*output$`% found`[2:3]
#   output$'# expected'[2:3] <-output$`% expected`[2:3]*output$`Nest UCI`[2:3]
#   #matchy math?
#   match.grid <- read.in[read.in[,1]!=0,]
#   asdf  <- superFUNlist(match.grid[match.grid[,2]!=0,2])[1]
#   output$matches[2] <- superFUNlist(match.grid[match.grid[,2]!=0,2])[1]
#   output$matches[3] <- superFUNlist(match.grid[match.grid[,3]!=0,3])[1]
#   #LOGIC TESTING
#   output$'in spec' <- output$matches > output$`# expected`
