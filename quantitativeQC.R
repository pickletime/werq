...
#df.working.file <- working.file this is for larger datasets. I'm just fucking around with a smaller subset right now.
...
#unique(df.working.file.called$Call) this exists just to make sure I have what I need. I think.
...
#unique(df.working.file.called$Call) again, not really sure what the point of this one is
...
#test
  df.called[df.called$SNPID == subset.called.snplist[i] & df.called$Call == subset.called.calls[j],10:18] <- subset.temp.calls[,10:18]
#test
...
#resetting so i can comparison?
df.master.list <- df.called
df.master.coord.x <- with(df.master.list,tapply(x.mean, list(SNPID = SNPID, Call = Call), mean))
df.master.coord.y <- with(df.master.list,tapply(y.mean, list(SNPID = SNPID, Call = Call), mean))
max(df.master.coord.x, na.rm = T)
