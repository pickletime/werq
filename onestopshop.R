...
file.list <- file.list[grepl("Genotyping",file.list)]
...
###this whole thing is for adding project number in, and i don't understand how it's so complicated.
  proj.number <- sub("Genotyping", "", unformatted.file)
  proj.number <- sub("-", "", proj.number)
  proj.number <- noquote(proj.number)
  proj.number <- sub("-", "", proj.number)
  #??
  proj.number <- substr(proj.number,1, nchar(proj.number)-2)
  #??
###this whole thing is for adding project number in, and i don't understand how it's so complicated.
...
{#I actually think this works now. No way it's that easy.
  ...
  for(i in 1:length(proj.list)){
    ...
    project.table[i,3] <- as.numeric(mean(loop.df.subset.qqc$distance))
    project.table[i,4] <- as.numeric(mean(loop.df.subset.qqc$distance.norm))
...
correct.names <- c("cust", "PlateID", "lysed.inits", "lysed.dates", "Inc.temp", "reag.pro", 
                   "lysis.buff", "clean.inits", "clean.dates", "reag.beads", "reag.buff", "reag.w1", "reag.w2", "reag.el")
#spec.cust <- c("FPL", "PFG", "Tyson Beef", "Tyson Pork", "Grind Flow", "A2", "CAB", "R&D", "Misc", "Chicken")
}
##obligatories
...
MGBD.names <- sheet.names[!grepl("IDBD", sheet.names)]
  ID003.IDBD <- read.xlsx(xlsxFile = path.file, sheet = IDBD.names, cols = c(1:12), rows = c(3:100000))
  colnames(ID003.IDBD) <- c("cust", "PlateID", "lysed.inits", "lysed.dates", "Inc.temp", "reag.pro", 
                            "reag.tween", "clean.inits", "clean.dates", "reag.beads", "reag.etoh", "reag.mol.el")
###

##for magbead
for(i in 1:length(MGBD.names)){
  if(i == 1){
    ID003 <- read.xlsx(xlsxFile = path.file, sheet = MGBD.names[i], cols = c(1:14), rows = c(2:100000))
    colnames(ID003) <- correct.names
    ID003$spec.cust <- MGBD.names[i]
  }else{
    input.file <- read.xlsx(xlsxFile = path.file, sheet = MGBD.names[i], cols = c(1:14), rows = c(2:100000))
    if(length(input.file) == 13){
      ...
      input.file.temp <- cbind(input.file.temp, input.file[,5:13])
...
##by job
#ID003.cleaned.cust %>% mutate(cust = fct_reorder(cust,clean.dates)) %>% 
